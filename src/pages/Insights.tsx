...
export default function Insights() {
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      {loading ? (
        <div className="text-center py-16 text-stone-500">Loading insights...</div>
      ) : stats ? (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-stone-900 rounded-3xl p-6 text-white">
              ...
              <div className="text-4xl font-extrabold text-white">${stats.totalPaidRevenue.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div>
              <div className="mt-2 text-sm text-stone-400">{stats.paidInvoices} paid invoice{stats.paidInvoices !== 1 ? 's' : ''}</div>
            </div>
            <div className="bg-white rounded-3xl p-6 border border-stone-100 shadow-sm">
              ...
              <div className="text-4xl font-extrabold text-amber-600">${stats.totalUnpaidRevenue.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div>
              <div className="mt-2 text-sm text-stone-400">{stats.unpaidInvoices} unpaid invoice{stats.unpaidInvoices !== 1 ? 's' : ''}</div>
            </div>
            <div className="bg-white rounded-3xl p-6 border border-stone-100 shadow-sm">
              <div className="flex items-center gap-2 mb-4">
              ...
              <div className="text-4xl font-extrabold text-stone-900">${(stats.totalPaidRevenue + stats.totalUnpaidRevenue).toLocaleString('en-US', { minimumFractionDigits: 2 })}</div>
              <div className="mt-2 text-sm text-stone-400">{stats.totalInvoices} total invoice{stats.totalInvoices !== 1 ? 's' : ''}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-3xl p-6 border border-stone-100 shadow-sm">
              <div className="flex items-center gap-2 mb-6">
                ...
                <h2 className="text-base font-bold text-stone-900">Inspections (All Time)</h2>
              </div>
              <div className="space-y-4">
                {[
                  ...
                  {
                    label: 'Other',
                    count: stats.totalInspections - stats.completedInspections - stats.scheduledInspections - stats.inProgressInspections,
